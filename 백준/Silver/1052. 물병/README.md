...
### 풀이 방법
 ...
 <p>2개를 합치는 방식이므로 처음 최대로 합치고 난뒤 모습은 이진법과 같은상황이다. 따라서 처음 입력받은 N을 이진법으로 바꾸고, 이 안에 1(물병)개수를 세서 들고 갈수 있는지 확인한다. 만약 불가능하다면 제일 작은 리터수 물병을 찾기위해 [2:]로 이진법 문자를 지우고 [::-1]로 뒤집어서 index하여 맨뒤부터 검색한다. 그렇게 찾은 1위치의 리터수만큼 물병을 구매하고, 구매하였으니 다시 십진수로 변환하고(안하고 N 그대로 이용하면됨) 여기에 구매한 물양만큼 더하고 다시 이진법으로 바꾼다. 이러면 물을 사고 알아서 2개씩 짝지어 합친결과물이 된다. 다시 검증하고 불가능하면 반복한다.</p>
